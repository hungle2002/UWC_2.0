...
const JanitorsAssigned = (props) => {
    ...
    const handleClick = () => {
        const url = `${props.info.mcpID}/janitor`
...
const MCPSelection = () => {
    ...
    useEffect(() => {
        let adds = []
        axios.get(`http://localhost:3000/api/mcp?route_id=${route}&week=11&month=12`)
        .then((res) => {
            setMcps(res.data)
        }
        )
        ...
        <div className="container-fluid" style={{height: "100vh"}}>
            <div className="row">
                ...
                <div className="col-4" style={{paddingTop: '30px'}}>
                        ...
                        <tbody>
                            {
                                mcps?.map((mcp, index) => (
                                    ...
                                    onClick={() => {
                                        ...
                                        mcp.janiator.split('/')[0] !== '0' ? setOpenList(mcps[index]) 
                                        : handleJanitorsList(mcp.mcpID, index) 
                                                                            
                                    }}>
                                        {
                                            mcp.janiator.split('/')[0] === mcp.janiator.split('/')[1] ?
                                            <td><img src={checkcircle}/></td> :
                                            <td><img src={crosscircle}/></td>
                                        }
                                        <td>{mcp.mcpID}</td>
                                        <td>{mcp.address}</td>
                                        <td>{mcp.janiator}</td>
                    ...
                    <Button variant="dark" onClick={() => navigate(-1)} style={{float: 'right'}}>
                        Quay về
                    </Button>
                </div>
            </div>   
